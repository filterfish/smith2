...
describe Smith::Messaging::AmqpOptions do
  ...
  before(:all) do
    @options = Smith::Messaging::AmqpOptions.new(:auto_delete => true)
  ...
  describe "with publish options" do
    before do
      @options = Smith::Messaging::AmqpOptions.new(:ack => true)
      @options.queue_name = "long.queue"
    end
    it 'should return the correct default publish options' do
      @options.publish.should == {:ack => true, :routing_key => "long.queue", :headers => {}}
    end
  end
  describe "with publish options" do
    before do
