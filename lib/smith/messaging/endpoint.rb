...
module Smith
  module Messaging
    class Endpoint
      ...
      attr_accessor :denormalised_queue_name, :queue_name

      def initialize(queue_name, options)
        @denormalised_queue_name = queue_name
      ...
      def ready(&blk)
        Smith.channel.direct(@queue_name, options.exchange) do |exchange|
          ...
          logger.verbose { "Creating queue: [queue]:#{denormalised_queue_name} [options]:#{options.queue}" }
      ...
      def messages?(blk=nil, err=proc {logger.debug { "No messages on #{@denormalised_queue_name}" } })
      ...
      def consumers?(blk=nil, err=proc {logger.debug { "Nothing listening on #{@denormalised_queue_name}" } })
